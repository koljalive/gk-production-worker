...
function Cards([long]$id,[string]$title){
 $key=if($title-match'WLAN|Wi.?Fi|Mesh|Repeater|Access Point|Frequenz|Routerstandort|Heimnetzwerk'){'wlan'}elseif($title-match'Router|FRITZ|Speedport'){'router'}elseif($title-match'Glasfaser|FTTH|ONT|Gf-|AKP|GPON|XGS.?PON|NVT|PoP|Speedpipe|Spleiß|Hauseinführung|Hausstich|OLT|Splitter|\bPON\b|Active Ethernet|Singlemode|Multimode|LC-Stecker|SC/APC|OTDR|Pegelmessung|Hausbegehung|Grundstückszustimmung|Eigentümererklärung|Hausanschluss|OXG|Westconnect|E\.ON|Anbieter gleichzeitig|Open Access|Ausbaugebiet|Vorvermarktung'){'fiber'}elseif($title-match'DSL|APL|TAE|TAL|KVz|HVt|Vectoring|CRC|FEC|Dämpfung|Störabstand|SNR|Resynchronisation|250 Mbit|Hausverkabelung|Leitung|Entstörung|Fehlerbilder'){'dsl'}elseif($title-match'Techniker|Termin|Keller|Außendienst|Schaltung|zuhause sein'){'termin'}else{return $null}
 ...
 $html += "</ul>`n</section>"; return $html
}
function Repair-Encoding([string]$html){
 if($html-notmatch'[ÃÂâð�]'){return $html}
 $map=[ordered]@{'Ã¤'='&#228;';'Ã¶'='&#246;';'Ã¼'='&#252;';'Ã„'='&#196;';'Ã–'='&#214;';'Ãœ'='&#220;';'ÃŸ'='&#223;';'â€“'='&ndash;';'â€”'='&mdash;';'â€ž'='&bdquo;';'â€œ'='&ldquo;';'â€™'='&rsquo;';'â‚¬'='&euro;';'Â '='&nbsp;'}
 foreach($key in $map.Keys){$html=$html.Replace($key,$map[$key])}
 if($html-notmatch'[ÃÂâð�]'){return $html.Replace('Â','')}
 $oldScore=[regex]::Matches($html,'[ÃÂâð�]').Count
 try{$candidate=[Text.Encoding]::UTF8.GetString([Text.Encoding]::GetEncoding(1252).GetBytes($html))}catch{return $html}
 $newScore=[regex]::Matches($candidate,'[ÃÂâð�]').Count
 if($candidate.Contains([char]0xFFFD)-or$newScore-ge$oldScore){return $html}
 return $candidate
}
