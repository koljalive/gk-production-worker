...
function Get-SchematicSources([string]$Html) {
    $sources = @(Get-ImageSources $Html)
    return @($sources | Where-Object { Is-Schematic $_ })
}
...
function Probe-Public([string]$Url, [string]$ExpectedImage = '') {
    try {
        ...
        return [pscustomobject]@{
            ok = $true; http = [int]$response.StatusCode; h1 = $h1; schematic = $schematicSources.Count;
            schematic_srcs = $schematicSources; expected_image = $expectedOk; image_count = $images.Count; image_srcs = $images
        }
    } catch {
        return [pscustomobject]@{
...
foreach ($candidate in $candidates) {
    ...
    try {
        ...
        $replacementMediaId = if ($needsImage) { [int]$replacementMedia.id } else { 0 }
        $plan = [pscustomobject]@{
            id = $id; type = $type; title = $title; slug = $slug; url = $url;
            original_content = $raw; original_featured_media = $featuredId;
            needs_h1 = $needsH1; raw_h1 = $rawH1; needs_image = $needsImage; category = $category;
            replacement_media_id = $replacementMediaId; replacement_url = $replacementUrl;
            replace_inline = $replaceInline; replace_featured = $replaceFeatured; pre_probe = $pre; backup_path = ''
        }
        $plans += @($plan)
        $result.plans += @([ordered]@{
            id = $id; type = $type; title = $title; url = $url; needs_h1 = $needsH1; needs_image = $needsImage;
            category = $category; replacement_media_id = $replacementMediaId; replace_inline = $replaceInline;
