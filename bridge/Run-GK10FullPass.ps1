...
function Probe([object]$z){
  ...
  $canonical=([regex]::Match($h,'<link[^>]+rel=["'']canonical["''][^>]+href=["'']([^"'']+)','IgnoreCase')).Groups[1].Value
  $viewport=($h -match 'name=["'']viewport["'']');$og=($h -match 'property=["'']og:image["'']');$noindex=($h -match 'noindex')
