...
$raw=[regex]::Replace($raw,'(?is)<!--\s*wp:image\b.*?<!--\s*\/wp:image\s*-->',[System.Text.RegularExpressions.MatchEvaluator]{param($m)if($m.Value-match'(?i)\.svg|exec-|schematic|diagramm|illustration'){''}else{$m.Value}})
$raw=[regex]::Replace($raw,'(?is)<figure\b[^>]*>.*?<\/figure>',[System.Text.RegularExpressions.MatchEvaluator]{param($m)if($m.Value-match'(?i)\.svg|exec-|schematic|diagramm|illustration'){''}else{$m.Value}})
...
if($raw-match('<!-- gk-direct-live-v1:'+[regex]::Escape($t.slug))){$pattern='(?is)<!-- gk-direct-live-v1:'+ [regex]::Escape($t.slug) +' -->.*?<!-- /wp:image -->';$raw=([regex]$pattern).Replace($raw,$block,1)}else{$raw=$block+$raw}
...
$change=$run.changed|Where-Object { $_.id -eq $t.id }|Select-Object -First 1;$stem=[IO.Path]::GetFileNameWithoutExtension(([uri][string]$change.image).AbsolutePath);$visible=$html-match[regex]::Escape($stem)
