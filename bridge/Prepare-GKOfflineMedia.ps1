...
$items=@()
foreach($m in $review.real_photo_candidates){$ext=$(if([string]$m.mime-eq'image/webp'){'.webp'}else{'.jpg'});$path=Join-Path $dir([string]$m.id+$ext);Invoke-WebRequest -Uri([string]$m.source_url)-OutFile $path-UseBasicParsing-TimeoutSec 180;if(-not(Test-Path $path)){throw "Media download missing $($m.id)"};$file=Get-Item $path;if($file.Length-lt1024){throw "Media file too small $($m.id) bytes=$($file.Length)"};$hash=(Get-FileHash $path -Algorithm SHA256).Hash;$items+=[ordered]@{id=[int]$m.id;title=[string]$m.title;alt=[string]$m.alt;source_url=[string]$m.source_url;local_path="bridge/offline-media-review/$($m.id)$ext";bytes=$file.Length;sha256=$hash;topics=$m.topics}}
