...
$rows=@();foreach($x in $items){$plain=Plain([string]$x.content.raw);$words=$(if($plain){($plain-split'\s+').Count}else{0});$rows+=[ordered]@{id=[int]$x.id;type=[string]$x.type;title=Plain([string]$x.title.rendered);slug=[string]$x.slug;key=Key([string]$x.slug);url=[string]$x.link;words=$words;modified=[string]$x.modified}}
