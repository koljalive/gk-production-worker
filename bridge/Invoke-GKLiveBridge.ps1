...
function Invoke-Wp {
    ...
    try {
        ...
        return [pscustomobject]@{ ok=$true; status=[int]$r.StatusCode; content_type=[string]$r.Headers['Content-Type']; body=[string]$r.Content; error=$null }
    }
    catch {
        $status=$null; $bodyText=''
        if ($_.Exception.Response) {
            try { $status=[int]$_.Exception.Response.StatusCode } catch {}
            try { $reader=New-Object IO.StreamReader($_.Exception.Response.GetResponseStream()); $bodyText=$reader.ReadToEnd(); $reader.Close() } catch {}
        }
        return [pscustomobject]@{ ok=$false; status=$status; content_type=$null; body=$bodyText; error=$_.Exception.Message }
    }
}
...
try {
    ...
    if ($action -eq 'post') {
        $allowedWrite = $path -match '^/wp/v2/(pages|posts)/\d+(\?.*)?$' -or $path -match '^/wp-abilities/v1/abilities/[A-Za-z0-9\-_/]+/run(\?.*)?$'
    ...
    try { $basic=[Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$wpUser`:$wpPass")) }
    finally { Remove-Variable wpPass -ErrorAction SilentlyContinue }
    ...
    if ($action -eq 'post' -and $path -match '^/wp/v2/(pages|posts)/(\d+)') {
        ...
        $stamp=Get-Date -Format 'yyyyMMdd-HHmmss'; $backupPath=Join-Path $backupDir "$type-$id-$stamp-before.json"
        [IO.File]::WriteAllText($backupPath,$before.body,(New-Object Text.UTF8Encoding($false)))
        $result.backup=$backupPath
        $result.preflight=[ordered]@{type=$type;id=$id;content_sha256=$beforeHash;modified=[string]$beforeObj.modified}
    }
    ...
    if($action -eq 'post' -and $path -match '^/wp/v2/(pages|posts)/(\d+)' -and ($request.body.PSObject.Properties.Name -contains 'content')){
        ...
        if((Get-Sha256 $actualRaw) -ne (Get-Sha256 $expectedRaw)){throw 'Verification hash mismatch after content write.'}
